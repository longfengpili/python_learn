...
### 参数
在编程中，函数参数可以有不同的类型，这些类型决定了如何将实际的值传递给函数。以下是一些常见的参数类型：

1. **位置参数（Positional Arguments）**：
   - 这些参数的值是根据它们在函数定义中的位置传递的。调用函数时，必须以正确的顺序提供这些参数的值。

2. **关键字参数（Keyword Arguments）**：
   - 这些参数在函数调用时通过“键=值”的形式传递，这意味着调用者指定每个参数的名称及其对应的值。这样可以不按照参数在函数定义中的位置来传递值。

3. **默认参数（Default Arguments）-也是关键字参数**：
   - 这些参数在函数定义时被赋予默认值。如果调用者没有提供值，函数将使用默认值。

4. **可变位置参数（Variable Positional Arguments）**：
   - 使用`*args`可以让函数接受任意数量的位置参数。这些参数被包装进一个元组中。

5. **可变关键字参数（Variable Keyword Arguments）-也是关键字参数**：
   - 使用`**kwargs`可以让函数接受任意数量的关键字参数。这些参数被包装进一个字典中。
   
```python
def example_func(positional_arg, *args, keyword_arg="default",  **kwargs):
    print("Positional argument:", positional_arg)
    print("Keyword argument with default:", keyword_arg)
...
example_func(
    ...
    "var positional 1",
    "var positional 2",
    keyword_arg="not default",
    keyword1="var keyword 1",
    keyword2="var keyword 2"
)
```

上面的例子中，`positional_arg`是一个位置参数，`*args`收集额外的位置参数，`keyword_arg`是一个带有默认值的关键字参数, 而`**kwargs`收集额外的关键字参数。在调用函数时，"positional"是位置参数的值，"keyword_arg"通过关键字指定了一个新值，其他的值被`args`和`kwargs`捕获。
